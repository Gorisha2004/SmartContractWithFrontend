...
export default function HomePage() {
  ...
  const increaseBalance = async () => {
    if (atm) {
        const amount = parseInt(prompt("Enter amount to increase:"), 10);
        try {
...
const decreaseBalance = async () => {
    if (atm) {
        const amount = parseInt(prompt("Enter amount to decrease:"), 10);
        try {
  ...
  const initUser = () => {
    ...
    return (
      <div>
        ...
        <button onClick={decreaseBalance}>Decrease Balance</button>
      </div>
    )
  }
